...
# OpenAI API
openai_token <- Sys.getenv("OPENAI_TOKEN_DIRK") # will cost about $9

# l1 responses
l1 <- read_csv("01_Data/Processed/l1_responses_v7.csv")
...
conditions <- l1 %>%
  ...
  mutate(prompt = paste0("What are the first five associations that come to a ",
                         ...
                         cue,
                         "'? Produce responses typical for a ",
...
for (j in 1:nrow(conditions)) {
  ...
  save(responses_list, file = paste0("01_Data/Raw/Increments/l2_responses_list_", sprintf("%04d", j), "_v7.RData"))
...
write_csv(result, "01_Data/Raw/l2_result_v7.csv")
...
write_csv(data, "01_Data/Processed/l2_responses_v7.csv")
